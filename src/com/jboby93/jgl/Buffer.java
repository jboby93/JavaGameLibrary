...
public class Buffer {
	...
	public void screenshot(String file) {
		...
		try {
			...
			LibInfo.dbg("Buffer.screenshot(): error saving screenshot file " + file + ": " + e.toString());
		}
	}
	...
	*/
	...
	public void drawText(String str, Point location, Color c, Font f) {
		buffer.setFont(f);
		buffer.setColor(c);
		int y = location.y - buffer.getFontMetrics().getHeight();
		
		for(String line : str.split("\n")) {
			buffer.drawString(line, location.x, y += buffer.getFontMetrics().getHeight());
		}
